...
export const POST = wrapCronHandler("stock-alert", async (req: Request) => {
  ...
  if (soldOut.length > 0) {
    ...
    for (const p of soldOut.slice(0, 8)) {
      // Nombre y referencia salen del catálogo del proveedor, donde "&" y "<>" son
      // habituales. Un solo producto con "&" tumbaba TODO el aviso de stock (Telegram
      // 400 y nadie se entera de los agotados).
