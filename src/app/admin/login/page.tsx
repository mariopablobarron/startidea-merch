...
export default async function LoginPage({
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-bone px-6">
      <form
        action={loginAction}
        className="w-full max-w-sm rounded-3xl border border-ink/10 bg-bone-soft p-8"
      >
        ...
        <label className="mt-6 grid gap-2">
          ...
          <input
            ...
            className="rounded-2xl border border-ink/15 bg-bone px-4 py-3 text-base outline-none transition focus:border-accent"
          />
        </label>
        {error && <p className="mt-3 text-sm text-red-600">⚠ Secret incorrecto</p>}
        <button
          type="submit"
