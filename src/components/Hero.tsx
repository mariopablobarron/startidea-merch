  ...
  return (
    <section className="relative overflow-hidden bg-bone">
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute -top-40 left-1/2 h-[600px] w-[1100px] -translate-x-1/2 rounded-full bg-accent/10 blur-3xl" />
      </div>

      <div className="mx-auto max-w-8xl px-6 pb-24 pt-20 lg:px-10 lg:pb-36 lg:pt-28">
        <p className="mb-8 inline-flex items-center gap-2 rounded-full border border-ink/10 bg-bone-soft px-4 py-1.5 text-xs font-medium uppercase tracking-wider text-ink/70">
          <span className="h-1.5 w-1.5 rounded-full bg-social" />
          Merchandising con impacto social real
        </p>

        <h1 className="max-w-5xl font-display text-hero font-semibold text-ink">
          Tu marca, en manos<br />
          que <span className="text-accent">cambian vidas</span>.
        </h1>

        <p className="mt-8 max-w-2xl text-lg text-ink/75 lg:text-xl">
        ...
        <dl className="mt-20 grid grid-cols-2 gap-8 border-t border-ink/10 pt-10 lg:grid-cols-4">
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div>
      <dt className="font-display text-4xl font-semibold tracking-tight text-ink lg:text-5xl">{value}</dt>
