...
async function cursarPedidoMidocean(
  ...
  if (!cart.shippingAddress || !cart.shippingPostalCode || !cart.shippingCity) {
    ...
    await notifyTelegram(
      ...
      // Telegram responde 400 y se pierde el único aviso de que hay un pago
      // cobrado cuyo pedido no puede salir.
  ...
  if (!result.ok) {
    ...
    await notifyTelegram(
      // Nombre del cliente + `result.error`, que es el cuerpo de error de la
      // API de MidOcean (JSON o HTML según el fallo): ambos traen `<` y `&`.
